...
// RJR: How to create files... Start with a raw binary, do not exceed max length,
//	disable literal sequences (-c), disable protocol bit 5 (-P-32 do not reuse
//	latest sequence offset), deactivate literal sequences (-T4, same as -c)
//
//	./prg2bin 01.prg 01.bin
//	./exomizer raw -T4 -M256 -P-32 -c -o 02.prg 01.bin

.filenamespace loader //continue use of loader namespace

        // Note: you need to define depackBuffer address (156 bytes)

.label tablBi          = depackBuffer
.label tablLo          = depackBuffer + 52
.label tablHi          = depackBuffer + 104

#define FORWARD_DECRUNCHING
#define LITERAL_SEQUENCES_NOT_USED
#define MAX_SEQUENCE_LENGTH_256

.label zpLenHi         = zpBitsLo	// set/computed in config.asm

// -------------------------------------------------------------------
// This source code is altered and is not the original version found on
// the Exomizer homepage. Forward decrunching mod#ifications improved
...
//   spec#ific prior written permission.
//
// -------------------------------------------------------------------
// no code below this comment has to be mod#ified in order to generate
...
// -------------------------------------------------------------------
        ...
        // Mod#ifies: A,X,Y,depacker ZP vars
...
begin_stx:
        stx zpBitsHi
    #if (FORWARD_DECRUNCHING && !MAX_SEQUENCE_LENGTH_256 && LITERAL_SEQUENCES_NOT_USED)
        bne no_next_round
        jmp next_round
no_next_round:
    #else
