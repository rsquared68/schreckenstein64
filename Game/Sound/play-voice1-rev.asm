/*	sound effects code for use in schreck irq handler
	...
	2023-12-26	reverse order of register loading so gate comes last
...
!reloadRegs:				//from beq:22, from jmp:22	*
	ldy #5				//2	load in reverse order so that gating is last
	lda (Voice1StreamPtr_zp),y	//5
	sta FREHI1			//4	use msb freq == 0 to end the sound effect wavetable
	sta halt1			//4	
	dey				//2
	lda (Voice1StreamPtr_zp),y	//5
	sta FRELO1			//4
