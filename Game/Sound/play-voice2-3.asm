...
!newData:				//		12 from beq
	ldy #0				//2				
	lda (Voice2StreamPtr_zpw),y	//5		This load will never cross a page boundary because the index y=0
	sta fxIteration2_zp		//3				  

!reloadRegs:				//from beq:22, from jmp:22	
	ldy #1				//2
	lda (Voice2StreamPtr_zpw),y	//5		These loads can result in a page boundary being crossed
	sta VCREG2			//4		e.g. if >ptr = $ff, every load will cross the page and 5 cycles are lost
	iny				//2
	lda (Voice2StreamPtr_zpw),y	//5
	sta ATDCY2			//4
	iny				//2
	lda (Voice2StreamPtr_zpw),y	//5
	sta SUREL2			//4
	...
	lda Voice2StreamPtr_zpw		//3		compute $ff-ptr lsb
	sec				//2
	sbc #$fb			//2		0 to 4 for $fb to $ff, want 7 cycles to 2 cycles added
ckd2:	sta variDly2			//4		PC + twos complement offset, so 0 is the next instruction after bcc below
...
!doVari:				//											16 cycles here
	bcc variDly2:*+2		//3		bit does not touch carry						19
...
!incPointer:					// 8 from beq		     
	// 6 elements per step							
	clc				//2
	lda Voice2StreamPtr_zpw		//3
	adc #6				//2
	sta Voice2StreamPtr_zpw		//3
	lda Voice2StreamPtr_zpw+1	//3
	adc #0				//2
