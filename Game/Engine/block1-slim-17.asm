...
// 6502bench SourceGen v1.8.5-dev1       
 /*
		...
		2025-01-08	v17 moved score, life, objective updates in here to speed up main service loop
 */
...
!TWO_PLAYERS:                  lda PlayerIndex_zp              
                               ...
                               jsr UPDATE_SCREEN_SCORE			// removed from main svc loop so only runs when needed
...
!UPDATE_LIFE:                  jsr UPDATE_SCREEN_LIFE	// removed from main svc loop so now only runs when needed, clobbers X
...
!START:                        stx !lifePoints-             
                               ...
                               bcc !CEILING_MAX+            
                               jmp !UPDATE_SCRN+		//update screen and exit                  
                                                         
!CEILING_MAX:                  lda #$c8                     	//else clamp at $c8
                               ldx !playerNum-              
                               sta PlayerLifeForce,x    
                              
!UPDATE_SCRN:                  jsr UPDATE_SCREEN_LIFE		// removed from main svc loop so now only runs when needed
...
!DO_DAMAGE:                    lda !stunCounter-            
                               ...
                               sta StunDelay,x    			                                          
                               lda !damagePoints-           
                               jsr INFLICT_DAMAGE_OR_KILL       //clobbers X
...
!STEAL:                        sec                          
			       ...
			       jsr UPDATE_SCREEN_OBJECTIVES	//moved out of main service loop so done only when necessary
...
!COMPLETE_LEVEL:               ldy #$01                     
                               ...
                               ldx #$28                     
                               lda #$00            // player 0         
                               jsr ADD_LIFE        // ADD_LIFE takes PlayerIndex from A. X is clobbered because ADD_LIFE updates screen now       
                               ldx #$28                     
                               lda #$01		   // player 1               
                               jsr ADD_LIFE        // clobbers X
...
// setting jump table so that the routines for levels 1 3, 4 get run                           
			       ...
			       sta SoundStackPtr_zp		//invalidate the queue it's filled with lightning   
                               ...
                               ldx #$14                     
                               lda PlayerIndex_zp              
                               jsr ADD_LIFE  			// clobbers X now for screen update
...
!USE_POTION:                   ldx #$0a                     
			       ...
			       ldx PlayerIndex_zp	// ADD_LIFE now clobbers X due to screen updater
			       sta p1Sound_zp,x											// load
